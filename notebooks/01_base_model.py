# -*- coding: utf-8 -*-
"""MachineLearningIncrementalCapstone.ipynb
...
- I started with some basic linear models—Linear Regression, Ridge, and Lasso—and they did okay. They explained about 48% of the patterns in bike rentals (R² ≈ 0.481), which gave me a decent starting point.
- Lasso stood out a bit—it dropped the less useful features and had the smallest errors (MAE and MSE), so it gave cleaner results.
- Things got really interesting when I added Polynomial Features (degree=2) and scaled everything properly. The models became smarter, capturing more complex patterns. Cross-validation showed R² scores around 0.623, so non-linear relationships clearly helped.


Making the Models More Stable

- At first, the polynomial models struggled to converge. I fixed that by adjusting regularization and letting them train for more iterations (max_iter). This was super important since polynomial features made things more complicated.

What Features Really Matter

- Lasso and Ridge help highlight the most useful features by shrinking the ones that don’t really contribute. So the ones that keep their weight probably have a strong impact on bike rentals.
- I haven’t plotted them yet, but I’d expect things like temperature, whether it’s a weekend, or what the weather’s like to be big players.


Business Implications:

- Staff allocation & maintenance can be optimized by forecasting peak rental hours.
- Dynamic pricing could be introduced during high-demand slots.
- Weather-based alerting can inform users or adjust fleet distribution dynamically.

Recommendations for Further Improvements

Here’s how you could take this project even further:

a) Model Enhancements
- Test non-linear tree-based models like RandomForestRegressor or XGBoostRegressor for possibly higher R² and robustness to outliers.
- Consider RandomizedSearchCV on those models to fine-tune hyperparameters.

b) Temporal Feature Engineering
- Extract and add features such as:
- Hour bins (e.g. Morning, Afternoon, Night)
- Day of week as dummy variables
- Interaction terms (e.g. Temperature × Humidity)

c) Visualization & Reporting
- Residual analysis to check for model bias.
- Visualize prediction accuracy with scatter plots or time series overlays.
- Create a business dashboard to monitor rental forecasts and key drivers.
